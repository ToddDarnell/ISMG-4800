...
{ var cookiestr, data="";
  ...
  for (var i=0; i < form.x.length; i++) 
         if(form.x[i].checked) data=form.x[i].value;
  

   cookiestr = form.name.value + '|' + data + '|' + form.quantity.value;
   //alert(cookiestr);
   if(data.length < 1) alert("Choose a product");
   else if(form.quantity.value.length < 1) alert("Specify a quantity");
