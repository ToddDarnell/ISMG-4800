...
function getCartData(name, delim)
{
  ...
  writeCart(nProd, pnameA, descriptA, quantityA, priceA, name, delim);
}
                   ...
                   quantityA, priceA, name, delim)
{
  ...
  { // if we have products create a table for them
    ...
    document.writeln('<input type="button" value="Update Cart" onclick="updateCartCookie(this.form,\'' + name + '\',\'' + delim + '\',.125);location.reload(true);">');
